...
class _HomeScreenAppState extends State<HomeScreen> {
  ...
  Future<void> _addNewItem(Map<String, dynamic> newItem) async {
    try {
      ...
      setState(() {
        items.add({...newItem, 'image': savedImage.path});
      });
      await _saveItems();
    } catch (e) {
  ...
  Future<void> _updateItem(Map<String, dynamic> updatedItem) async {
    try {
      ...
      final favIndex = favoriteItems.indexWhere(
        (item) => item['id'] == updatedItem['id'],
      );
      if (favIndex != -1) {
        setState(() {
  ...
  List<Map<String, dynamic>> get filteredItems {
    return items.where((item) {
      final tagMatch =
          selectedTag == null || (item['tag'] as List).contains(selectedTag);
      final colorMatch =
          selectedColor == null ||
          (item['color'] as String).toLowerCase() ==
                ...
                IconButton(
                  onPressed: () {
                    Navigator.push(
                      ...
                      MaterialPageRoute(
                        builder:
                            (context) => AddItemPage(onItemAdded: _addNewItem),
                      ),
                    );
                  },
